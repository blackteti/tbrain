...
export async function GET() {
    try {
        ...
        const [brRes, worldRes] = await Promise.all([
            parser.parseURL(BR_RSS).catch((e: any) => { console.error("[NewsService] Falha no BR RSS:", e.message); return { items: [] }; }),
            parser.parseURL(WORLD_RSS).catch((e: any) => { console.error("[NewsService] Falha no World RSS:", e.message); return { items: [] }; })
        ]);

        const formatNews = (items: any[], source: string) => {
        ...
        return NextResponse.json({ global: [], timestamp: new Date().toISOString() }, { status: 500 });
    }
}
